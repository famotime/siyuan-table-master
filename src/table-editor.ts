...
export class TableEditor {
  ...
  async splitAllCells(): Promise<void> {
    ...
    const rawKramdown = this.ctx.getRawKramdown();
    if (/^\s*<table\b/i.test(rawKramdown)) {
      const parsed = convertHtmlTableToMarkdownKramdown(rawKramdown);
      if (parsed) {
        this.ctx.setTableModel(parsed.tableLines, parsed.ialLine);
        await this.ctx.flush();
        return;
      }
    }
...
/** 将 HTML 合并单元格展开为完整矩形网格。 */
function removeHtmlTableMergeAttributes(kramdown: string): string {
  const activeSpans: Array<ActiveHtmlSpan | null> = [];
  let knownColumnCount = 0;

  return kramdown.replace(/<tr\b([^>]*)>([\s\S]*?)<\/tr>/gi, (_row, rowAttrs: string, rowHtml: string) => {
    const sourceCells = Array.from(rowHtml.matchAll(/<(td|th)\b([^>]*)>([\s\S]*?)<\/\1>/gi));
    ...
    const appendActiveSpan = () => {
      const active = activeSpans[column];
      if (!active) return false;
      outputCells.push(`<${active.tagName}></${active.tagName}>`);
      active.remainingRows--;
      if (active.remainingRows === 0) activeSpans[column] = null;
      column++;
      return true;
    ...
    for (const match of sourceCells) {
      while (appendActiveSpan()) {
        // 当前源码行会省略被上一行 rowspan 占用的单元格。
      }

      const tagName = match[1].toLowerCase();
      const attributes = match[2];
      const content = removeMergeAttributes(match[3]);
      const colSpan = getHtmlSpan(attributes, "colspan");
      const rowSpan = getHtmlSpan(attributes, "rowspan");
      const remainingAttributes = attributes.replace(
        /\s+(?:colspan|rowspan)\s*=\s*(?:"[^"]*"|'[^']*'|[^\s>]+)/gi,
        "",
      );

      outputCells.push(`<${tagName}${remainingAttributes}>${content}</${tagName}>`);
      for (let offset = 1; offset < colSpan; offset++) {
        outputCells.push(`<${tagName}></${tagName}>`);
      }
      if (rowSpan > 1) {
        for (let offset = 0; offset < colSpan; offset++) {
          activeSpans[column + offset] = { remainingRows: rowSpan - 1, tagName };
        }
      }
      column += colSpan;
    }
    ...
    while (column < targetColumnCount) {
      if (!appendActiveSpan()) {
        const tagName = sourceCells[0]?.[1]?.toLowerCase() || "td";
        outputCells.push(`<${tagName}></${tagName}>`);
        column++;
      }
    }
    knownColumnCount = Math.max(knownColumnCount, outputCells.length);
