...
- 适用版本：SiYuan `v3.7.3` / `siyuan` npm `v1.2.3`
- 最后核对：2026-08-02
- 稳定性：stable（含部分历史事件）
- 权威来源：
...
详细事件清单与版本对照见：`../02-plugin-api/事件总线事件清单与版本差异.md`
...
- 同步与系统：
  ...
  - `lock-screen`
  - `kernel-plugin-state-change`（等待内核插件 RPC 就绪）
- 代码块语言（新增）：
...
- 升级思源后，先对照 npm `siyuan` 类型、`petal` 变更记录和运行时 `TEventBus` 查看新增或变更事件
- 插件中对非核心事件加“存在性降级”逻辑
